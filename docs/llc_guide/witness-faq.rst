...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
You should always make sure you use the newest build available `here <https://github.com/localcoinis/localcoin-core/releases>`_
...
Whose private key is `["LLC6MRyAjQ..","5KQwrPbwdL.."]`? Why is it predefined in the `config.ini`?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
