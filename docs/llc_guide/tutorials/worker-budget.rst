...
workers where reserve fund is how many LLC are currently not distributed (see
the `source code`_).
...
LLC are available to be distributed and how many LLC will be returned to the
...
(i.e., 1.5 LLC per block is budgeted, so budget remaining is 

::

    1.5 LLC * (number of blocks left in maintenance period+blocks missed by witnesses))
