...
You can create a Worker Proposal by using LocalCoin UI form. Go to *Voting* page. There are three tabs: "Witnesses", "Committee", and "Workers". In the "Worker" tab, you will find *CREATE A NEW WORKER* button.
...
An `owner_account` is creating a one day worker starting Oct 28 and will get paid 1 LLC/day (vesting in 1 day, 1 LLC = 100,000 ‘satoshi’) to make an android app. The first command won’t broadcast, this will just check::
...
The variable ``pay_vesting_period_days`` is the integer number of days you set for vesting. Some people don’t want workers to withdraw and sell large sums of LLC immediately, as it puts sell pressure on LLC. Also, if you require vesting, you have “skin in the game” and thus an incentive to improve LLC value. Pay is pay per day (not hour or maintenance period) and is in units of 1/100,000 LLC (the precision of LLC)
...
You can check the Worker Proposals by LocalCoin UI form. Go to *Voting* page. There are three tabs: "Witnesses", "Committee", and "Workers". In the "Worker" tab, you will find all Worker Proposals list.
...
You also can inspect all the objects 1.4.*::
    ...
    [{
        ...
        "url": "https://localcointalk.org/index.php/topic,19317.0.html"
      }
    ]
...
for example::
    ...
    >>> update_worker_votes "awesomelocalcoinr" {"vote_for":["1.4.0"]} true
...
So the daily budget is::
    ...
    worker_budget*24 = 1340913100 * 24 = 32181914400 (or 321,8191.44 LLC)

There is currently a maximum daily worker pay of 500k LLC, and this can be found using the `get_global_properties` command in the cli_wallet.
...
is allocated for witnesses and workers. The reserve fund is maximum number of LLC available less those currently in circulation ([source](https://github.com/cryptonomex/graphene/blob/f85dec1c23f6bf9259ad9f15311b2e4aac4f9d44/libraries/chain/include/graphene/chain/config.hpp))

Every hour the total available reserve fund is calculated by finding how many LLC are available to be distributed and how many LLC will be returned to the reserve fund (i.e., “burnt”) during the next maintenance interval.

First find how many LLC have not been distributed::

    >>> from_initial_reserve = max_supply - current supply of LLC
...
Modify it by adding the accumulated fees and witness budget remaining (i.e., 1.5 LLC per block is budgeted, so budget remaining is 1.5 LLC * (number of blocks left in maintenance period+blocks missed by witnesses)) in this maintenance cycle (they will be added to the “reserve fund” permanently at maintenance)::
